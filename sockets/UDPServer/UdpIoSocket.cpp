 ...
 * =====================================================================================
 */
#include "sockets/udp/server/UdpIoSocket.h"
...
bool UdpIoSocket::recvfrom(UdpIoRequest * request)
{
    ...
    while(!mReleasing && mRecving)
    {
        ...
        receivedBytes = ::recvfrom(mFd, buffer, sizeof(buffer), flags, (sockaddr*)&srcAddr, &addrLen);
        if(0 >= receivedBytes)
        {
            int anErrno = errno;
            handleError(anErrno);
        ...
        //printf("recv: %s, receivedBytes = %ld\n", buffer, receivedBytes);
...
bool UdpIoSocket::sendto(UdpIoRequest * request)
{
    ...
    socklen_t addrLen = sizeof(struct sockaddr_in);
    ::memset(&srcAddr, 0, addrLen);
    srcAddr.sin_addr.s_addr = ::inet_addr(request->Args.Sendto.mIP.c_str());
    srcAddr.sin_port = htons(request->Args.Sendto.mPort);
    ...
    while(0 < mFd && sentBytes < request->Args.Sendto.mBufferSize)
    {
        ssize_t aSentSize = ::sendto(mFd, request->Args.Sendto.mBuffer,
                ...
                (const sockaddr*)&srcAddr, addrLen);
        if(0 >= aSentSize)
        {
            handleError(errno);
            if(-1 == aSentSize)
